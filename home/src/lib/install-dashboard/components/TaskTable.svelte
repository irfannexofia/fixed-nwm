<script lang="ts">
  ...
  const props = $props<{ storageKey?: string; scope?: string }>();
  ...
  async function load() {
    try {
      const res = await fetch(`/api/briefs/${props.scope ?? 'ugc'}`);
  ...
  function attachSse() {
    try {
      const scope = props.scope ?? 'ugc';
      sse = new EventSource(`/api/stream?channel=briefs:${scope}`);
      const reload = () => { load(); };
      sse.addEventListener('briefs/create', reload as any);
      sse.addEventListener('briefs/delete', reload as any);
    } catch (e) {
      console.warn('SSE connect failed', e);
    }
  }
  onMount(() => { load(); attachSse(); });
