<script lang="ts">
  ...
  const props = $props<{ title?: string; subtitle?: string; compact?: boolean; boardSlug?: string }>();
  function getBoardSlug() { return props.boardSlug ?? 'work'; }
  ...
  async function loadTasks() {
    try {
      const res = await fetch(`/api/kanban/${getBoardSlug()}/tasks`);
  ...
  function attachSse() {
    try {
      sse = new EventSource(`/api/stream?channel=kanban:${getBoardSlug()}`);
      const reload = () => { loadTasks(); };
      sse.addEventListener('kanban/create', reload as any);
      sse.addEventListener('kanban/update', reload as any);
      sse.addEventListener('kanban/delete', reload as any);
    } catch (e) {
      console.warn('SSE connect failed', e);
    }
  }
  onMount(() => { loadTasks(); attachSse(); });
  onDestroy(() => { try { sse && sse.close(); } catch {} sse = null; });
...
<div class="kb-board space-y-6" class:compact={props.compact ?? false}>
  <div class="kb-header" style="display:flex;align-items:center;justify-content:space-between;gap:12px">
    <div>
      <h1 class="kb-title">{props.title ?? 'Work Management'}</h1>
      <p class="kb-subtitle">{props.subtitle ?? 'Track tasks across your team and projects'}</p>
    </div>
